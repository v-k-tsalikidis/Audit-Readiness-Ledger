...
def test_json_session_roundtrip():
    ...
    session.entries[
        "A.7.2"
    ].root_cause = "Badge reader was decommissioned during office renovation."
    session.entries[
        "A.7.2"
    ].corrective_action = "Install biometric access control lock on server room."
...
def test_re_audit_verification_closes_blocker():
    ...
    _assessment1, gate_eval1 = session.evaluate_assurance()
    ...
    session.entries[
        "A.7.2"
    ].verification_notes = "Physical inspection confirmed new card reader active with tamper alarm."
